...
void send_msg(unsigned int x, unsigned int y, unsigned int z) {
    ...
    if( error == 0 )
    {
        ...
        data = (char *)malloc((strlen(MAC) + 3) * sizeof(char));
        strcpy(data, MAC);
        strcat(data, code);
        ...
        if (flag == 1)
        {
            ...
            if (flag == 0)
            {
                ...
                while (true)
                {
                    ...
                    {
                        USB.print(F("x = "));
                        USB.print(10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[17] << 8) | BLE.event[16]);
                        USB.print(F(", y = "));
                        USB.print(10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[19] << 8) | BLE.event[18]);
                        USB.print(F(", z = "));
                        USB.print(10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[21] << 8) | BLE.event[20]);
                        USB.println();
                        x = 10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[17] << 8) | BLE.event[16];
                        y = 10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[19] << 8) | BLE.event[18];
                        z = 10 * (unsigned long)sensorMpu9250AccConvert((uint16_t)BLE.event[21] << 8) | BLE.event[20];
